...
\parab{Payload copy.}
In most socket systems, the semantics of \texttt{send} and \texttt{recv} cause memory copies between application and network stack. For non-blocking \texttt{send}, the system needs to copy data out of the buffer because the application may overwrite the buffer right after \texttt{send} returns. %Simply removing these copies may violate application correctness. 
...
None of existing works support sharing a socket among threads and processes, causing compatibility problems in fork and container live migration, and multi-thread locking overhead as well.
First, when a process forks, only one of the parent and child processes can use existing sockets, and the choice needs to be pre-configured statically.
However, many web services~\cite{apache,php-fpm,python-gunicorn,vsftpd} have a master process to accept a connection and read the request header, then it may fork off a child process to handle the request, where the child needs to access the socket.
