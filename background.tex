...
\begin{table}[t]
	...
	\scalebox{0.8}{
		\begin{tabularx}{0.6\textwidth}{l|X|X}
			...
			\hline
			\hline
			Per op & Kernel crossing (syscall) & User space library (\S\ref{sec:architecture}) \\
			\hline
			Per op & Socket FD locks for concurrent threads \& processes & Lock-free socket sharing (\S\ref{subsec:fork}) \\
			\hline
			\hline
			Per packet & Transport protocol (TCP/IP) & Use RDMA / SHM (\S\ref{subsec:lockless-queue}) \\
			\hline
			Per packet & Buffer management & New ring buffer (\S\ref{subsec:lockless-queue}) \\
			\hline
			Per packet & I/O multiplexing & Use RDMA / SHM (\S\ref{subsec:lockless-queue}) \\
			\hline
			Per packet & Interrupt handling & Event notification (\S\ref{subsec:process-mux}) \\
			\hline
			Per packet & Process wakeup & Event notification (\S\ref{subsec:process-mux}) \\
			\hline
			\hline
			Per byte & Payload copy & Page remapping (\S\ref{subsec:zerocopy}) \\
			\hline
			\hline
			Per conn. & Kernel FD allocation & FD remapping table (\S\ref{subsec:connection-management}) \\
			\hline
...
\label{subsec:per-operation-overhead}
\quad
...
As shown in Table~\ref{tab:operation-performance}, the hairpin latency from CPU to NIC is much higher than inter-core cache migration delay.
...
LibVMA~\cite{libvma}, OpenOnload~\cite{openonload}, DBL~\cite{dbl} and LOS~\cite{huang2017high} are conform to the standard socket API.
...
RSocket uses the NIC to forward packets between two applications in a same host, so the latency and throughput is limited similar to Arrakis.
