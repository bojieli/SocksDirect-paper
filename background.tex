...
Several works~\cite{martins2014clickos,roghanchi2017ffwd,huang2017high} delegate operations to a virtual switch running on dedicated cores, making the virtual switch a bottleneck. Our goal is to achieve kernel bypass with process isolation, while not delegating every operation to a centralized coordinator.
...
Linux kernel acquires global locks during connection creation and a per-socket lock for each socket operation~\cite{boyd2010analysis,han2012megapipe,lin2016scalable}. Previous works~\cite{boyd2010analysis,clements2015scalable} suggest that many socket operations are not commutable and thus synchronization cannot always be avoided. For example, synchronization is required when a socket is shared after fork. In \sys, we try to minimize synchronization overhead in two ways. First, we optimize for the common case and remove synchronization overhead from frequent socket operations. Second, we leverage the fact that shared memory message passing is much cheaper than locking, and use message passing as the exclusive synchronizing mechanism.
...
\parab{Memory copy.}
Socket APIs, \textit{e.g.}, \texttt{send} and \texttt{recv}, cause memory copies between user and kernel space, thus degrading the performance. In \sys, we aim to achieve zero copy for large transfers without changing applications. We achieve this by using the page mapping mechanism in virtual memory.  
