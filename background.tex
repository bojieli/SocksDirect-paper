...
Summarize performance impairments/requirements here. 
...
Violate goal 2: context switch overhead, +latency for syscall batching 

Violate goal 4: Large memory footprint for many connections 

User-space stack 

New abstraction (RDMA, lwip + DPDK etc.) 

Some works violate goal 3: data transmission via centralized virtual switch 

Compatible with socket (libvma etc.) 

Violate goal 2: memory copy 

Violate goal 3: thread synchronization for multi-thread applications 

Common problems: 

Designed for networking, does not support or optimize for inter-process communication 

Violate goal 4: Not optimized for many connections 
