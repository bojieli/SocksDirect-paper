...
\begin{table}[t]
	...
	\scalebox{0.8}{
		\begin{tabularx}{0.6\textwidth}{l|X|X}
			...
			\hline
			Per op & Per socket locks for concurrent threads \& processes & Threads as a shared-nothing distributed system (\S\ref{subsec:fork}) \\
			\hline
			\hline
			...
			\hline
			Per packet & NAT \& ACL for container network & Control \& data plane separation (\S\ref{subsec:architecture}) \\
			\hline
			\hline
