...
Modern data centers have networks with microsecond scale latency and tens of Gbps throughput, making socket a well-known bottleneck for communication intensive applications.
...
Additionally, a server in data center contains tens of cores, therefore multiple applications and containers run on a same host, making intra-host socket communication increasingly important.
...
(2) a reliable and ordered communication channel, identified by an integer \emph{file descriptor} (FD);
(3) multiplex events from multiple channels.
Most Linux applications use a readiness-driven I/O multiplexing model, in which the OS tells application which FDs are ready to receive or send, then the application may prepare buffers and issue socket operations.
...
In addition, because a socket connection provides a FIFO abstraction and the OS offers event multiplexing, it is common for two processes to create multiple connections between them, where each connection handles a concurrent task.
...
Sockets can even be passed to another process through UNIX domain socket.
...
Many user-space sockets provide both standard socket API and an alternative zero-copy API, but none of them achieve zero copy for the standard API on both send and receive paths.
