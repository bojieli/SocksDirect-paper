...
\subsection{Overheads of Linux Socket}
...
\begin{table}[t]
	\centering
	\scalebox{0.8}{
	\begin{tabularx}{0.58\textwidth}{l|X|X}
		\hline
		Type & Overhead & Our Approach \\
		\hline
		\hline
		Per op & Kernel crossing & User space library (\S\ref{subsec:architecture}) \\
		\hline
		Per op & NAT \& ACL for container network & Control \& data plane separation (\S\ref{subsec:architecture}) \\
		\hline
		Per op & Per socket lock & Threads as a share-nothing distributed system (\S\ref{subsec:fork}) \\
		\hline
		\hline
		Per conn. & Lock in TCB management & Monitor daemon (\S\ref{subsec:connection-management}) \\
		\hline
		Per conn. & Lock in FD allocation & User space library  (\S\ref{subsec:connection-management}) \\
		\hline
		Per conn. & VFS abstraction & User space library (\S\ref{subsec:connection-management}) \\
		\hline
		...
		\hline
		Per packet & Buffer management & New queue design (\S\ref{subsec:lockless-queue}) \\
		\hline
		Per packet & I/O multiplexing & Offload to RDMA NIC \\
		\hline
		Per packet & Interrupt handling & Polling (\S\ref{subsec:process-mux}) \\
		\hline
		...
		\hline
		Per byte & Checksum calculation & Offload to RDMA NIC \\
		\hline
	\end{tabularx}
	}
...
\RED{Analyze socket system here...}

\parab{Per operation overheads:}

\parab{Per connection overheads:}

\parab{Per packet overheads:}

Transport protocol: packet parsing, congestion control, loss recovery, timer management...

\parab{Per byte overheads:}
...
Traditionally, socket APIs are implemented in kernel, thus requiring kernel crossing for each socket operation. To make it worse, the Kernel Page-Table Isolation (KPTI) patches~\cite{kpti} to protect against Meltdown~\cite{Lipp2018meltdown} attacks make kernel crossings 4x expensive.
...
Linux kernel acquires several global locks during connection creation~\cite{boyd2008corey,boyd2010analysis}.
MegaPipe~\cite{han2012megapipe} and FastSocket~\cite{lin2016scalable} remove these bottlenecks, but leave other kernel overheads on the table~\cite{jeong2014mtcp}. %.\RED{What are problems of MegaPipe and FastSocket}
