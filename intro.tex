...
There has been extensive work (Sec.~\ref{subsec:related-work}) aiming at reducing the socket overhead, but existing approaches are not satisfactory.
...
Second, even in the performance front, there are still much room for improvement. None of existing works can achieve performance close to raw RDMA, because they fail to remove some important overheads such as multi-thread synchronization, buffer management, memory copy and TCP/IP protocol processing.
...
At its heart, \sys{} leverages RDMA and \emph{shared memory} (SHM) for inter-host and intra-host transports, respectively.
...
We aim to translate socket to RDMA and SHM efficiently.
...
We use RDMA or shared memory to synchronize from sender to receiver.
