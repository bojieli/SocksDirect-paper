...
They either optimize the kernel networking stack~\cite{lin2016scalable,han2012megapipe,yasukata2016stackmap}, or build a user-space TCP/IP stack~\cite{jeong2014mtcp,marinos2014network,seastar,fstack,libvma}, or offload the transport to RDMA NICs~\cite{rsockets,socketsdirect}.
However, existing approaches have limitations on compatibility and performance.
First, many of them are not fully compatible with native socket in areas such as process fork, event polling, multiple application sharing and intra-host communication. 
...
Second, even in the performance front, there is still much room for improvement. None of existing works can achieve performance close to raw RDMA, because they fail to remove some important overheads such as multi-thread synchronization, buffer management and memory copy.
The key problem is that socket provides a virtual file system (VFS) abstraction to applications, and the bulk of overhead in existing socket systems is to provide this abstraction rather than the TCP/IP protocol~\cite{clark1989analysis,boyd2010analysis,jeong2014mtcp}.
...
Although the VFS abstraction of socket is complicated and not scalable in general~\cite{clements2015scalable}, many frequently used operations are simple and scalable.
Our principle is to optimize for the common cases and be compatible with all cases.
To speedup data transmission while preserving isolation in connection management, \sys{} follows the principle of separating control and data plane~\cite{peter2016arrakis}.
...
On the data plane, we exploit multiple techniques to effectively utilize hardware and improve system efficiency.
