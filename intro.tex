...
The first challenge is how to share a socket among threads and processes without locking. A socket connection is a FIFO channel. 
The most straight-forward approach is to implement each connection as a queue. Unfortunately, a socket connection may be shared by multiple 
senders and receivers, thus we need locking to protect the shared queue, which significantly slows down the system during 
...
FIFO semantics during fork and thread creation. The second challenge is how to maintain performance with vast number of concurrent connections. 
...
We exploit many techniques to effectively utilize hardware to support efficient communication. 
