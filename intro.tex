...
Realizing these limiations, we design \sys{}, a user-space socket system to achieve compatibility, isolation and high performance simultaneously.
\begin{ecompact}
...
To achieve the above goals, we first need to understand the overhead of Linux socket. We find that Linux socket provides a virtual file system (VFS) abstraction to applications, and the bulk of overhead in existing socket systems is due to this abstraction rather than TCP/IP protocol~\cite{clark1989analysis,boyd2010analysis,jeong2014mtcp}.
Although the VFS abstraction is complicated and not scalable in general~\cite{clements2015scalable}, many frequently used socket operations are actually simple and scalable.

Therefore, our design principle is optimizing for the common situation while staying compatible in general.
