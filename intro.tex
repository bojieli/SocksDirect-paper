...
%Those solutions that do support multiple applications often incur the overhead of kernel crossing or virtual switch.
Second, even in the performance front, there are still much room for improvement. For example, none of existing works can achieve performance close to raw RDMA, because they fail to remove some important overheads such as multi-thread synchronization, memory copy and TCP/IP protocol processing.
