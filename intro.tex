...
\textcolor{red}{Many of them have isolation problems. More details here}
Even in the performance front, there is still much room for improvement. None of existing work can achieve performance close to raw RDMA, because they fail to remove some important overheads such as synchronization, buffer management and memory copy. For example, \textcolor{red}{Explain the overhead of memory copy here}
...
To achieve high performance, \sys fully exploits the capability of modern hardware. It leverages RDMA for inter-host communication and uses \emph{shared memory} (SHM) for intra-host communication. However, how to translate socket functions to high performance RDMA and shared memory operations while still preserving compatibility is challenging. Straightforward solutions may violate the compatibility. For example, \textcolor{red}{Take socket send() to RDMA send operations as an example here. If no memory copy, compatibility. If memory copy, performance degradation. Btw, how to point out isolation problem here?}

To achieve all the three goals simultaneously, we first need to understand how Linux socket provides compatibility and isolation. Linux socket provides a virtual file system (VFS) abstraction to applications. With this abstraction, application developers can do communication operations like operating files, without digging into network protocol details. This abstraction also provides good isolation \textcolor{red}{TBA}. However, the VFS abstraction is very complicated givens its generality of cover many corner cases, thus incurring high overhead~\cite{clark1989analysis,boyd2010analysis,jeong2014mtcp}.   
   
Wwe find that many common used socket operations are actually simple. despite the generality and complexity of VFS. Therefore, our design principle is optimizing for the common situations while staying compatible in general.
To speedup data transmission while preserving isolation in connection management, \sys{} separates control and data plane~\cite{peter2016arrakis}.
We design a \emph{monitor} daemon in each host as the \emph{control plane} to enforce access control policies, dispatch new connections, perform address translation for container networks, and establish transport channel between communication peers.
...
In \libipc{}\textcolor{red}{inconsistent name!}, data transmission and event polling is handled in a peer-to-peer fashion between processes
