...
There exists an extensive body of work aiming at reducing the socket overhead.
...
Even in the performance front, there is still much room for improvement. None of existing work can achieve performance close to raw RDMA, because they fail to remove some important overheads such as synchronization, buffer management and memory copy.
...
A key problem for socket system is that socket provides a virtual file system (VFS) abstraction to applications, and the bulk of overhead in existing socket systems is to provide this abstraction rather than the TCP/IP protocol~\cite{clark1989analysis,boyd2010analysis,jeong2014mtcp}.
Although the VFS abstraction is complicated and not scalable in general~\cite{clements2015scalable}, many frequently used socket operations are simple and scalable.
Our principle is to optimize for the common situation while stay compatible in general.
...
In \libipc{}, data transmission and event polling is handled in a peer-to-peer fashion between processes
