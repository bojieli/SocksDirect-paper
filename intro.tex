...
There has been extensive work aiming at reducing the socket overhead.
These approaches still have limitations.
First, many user-space sockets are not fully compatible with existing applications, e.g., fork(), multiple processes listen a same port, and intra-host communication among containers and applications.
Second, several user-space sockets have security problems because applications can directly access shared NIC queues, thus violate process isolation and cannot enforce access control policies.
Other works preserve security, but incur the overhead of kernel or virtual switch.
Third, none of existing works achieve performance close to bare-metal RDMA because they fail to remove some of the important overheads such as multi-thread synchronization, memory copy and TCP/IP packet processing.
...
We design \sys{}, a user-space socket architecture with compatibility, security and performance.
\begin{ecompact}
	\item \textbf{Compatibility}.
	First, existing applications can use \sys{} as a drop-in replacement with no modification.
	Second, it supports both intra-host and inter-host communication.
	Third, it behaves correctly with fork or thread creation.
	Finally, if the remote peer does not support \sys{}, it falls back to kernel TCP socket transparently.
	\item \textbf{Security}.
	\sys{} preserves isolation among applications and containers, as well enforces firewall rules and access control policies.
	\item \textbf{High Performance}.
	\sys{} delivers consistent high throughput and low latency that is scalable with number of CPU cores, concurrent connections and threads.
\end{ecompact}
...
The data plane is composed of applications with a dynamically loaded user-space library \libipc{}.
%Applications can take advantage of \libipc{} by simply using the \emph{LD\_PRELOAD} environment variable in Linux to load the library.
\libipc{} intercepts Linux \emph{glibc} APIs, implements socket APIs in user space and forwards other APIs to the kernel.
\libipc{} delegates connection creation to the local monitor, while implements data transmission and event polling peer-to-peer to achieve scalable performance.


However, we face three performance challenges.
...
To avoid locking overhead, we treat each thread as a separate process, even if the threads have shared memory address space.
Therefore, all threads compose a shared-nothing distributed system that communicates via peer-to-peer message queues.
