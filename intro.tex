...
We design \sys{}, a high performance user-space socket architecture that is compatible with existing applications and preserves isolation among processes, while being scalable to multiple cores and many concurrent connections.
...
The main component of \sys{} is a user-space library \libipc{} that intercepts and implements Linux socket APIs. Applications can take advantage of \libipc by simply using the \textit{LD\_PRELOAD} environment variable in Linux to load the library.
...
We set out to build \sys{} with four performance goals.
...
Moreover, the performance should be \textit{scalable} to multiple processes on modern multi-core CPUs, especially when the number of threads exceed the number of cores. Finally, the performance should not degrade with large number of \textit{concurrent connections} between two processes.
...
Data plane (e.g. send/recv) is transferred peer-to-peer to achieve scalable performance.
...
For each pair of processes, \sys multiplexes socket connections through one queue.
\textbf{How to solve head of line blocking?}
...
First, we design different queue structures for shared memory and RDMA transports.
We use one-sided RDMA write and shared CQ to amortize polling overhead.
...
To share a CPU core efficiently among multiple active processes or threads, observing the event-driven behavior of applications, \sys uses \emph{cooperative multitasking} to remove kernel event processing overhead.
...
\sys{} achieves latency and throughput that is close to the raw performance of shared memory and RDMA.
...
\textbf{The main contributions of this paper are summarized as follows:}
