...
For inter-server communication, to save RDMA messages, the sender uses a single RDMA message to write a ring buffer message and clear header of the next message. Because RDMA does not ensure write ordering within a message~\cite{infiniband2000infiniband}, sender uses \textit{RDMA write with immediate} verb and receiver polls the RDMA completion queue rather than the ring buffer.
...
For inter-server connections, we use the RDMA NIC as the multiplexer. Receiver uses a \textit{shared completion queue} with all hosts, so it only needs to poll one queue rather than multiple ring buffers.
