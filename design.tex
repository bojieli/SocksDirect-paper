...
If the peer does not support \sys{}, all future operations are proxied and translated by the local monitor.
...
The server monitor selects one application from all that listen on a same address and port, then forwards the SYN to server application.
This differs from Linux where the new connection requests are queued in a \emph{backlog} in the kernel.
In the Linux design, every time the server \texttt{accept}s a new connection, it accesses the kernel to dequeue a connection from the backlog, which requires synchronization and adds latency.
In contrast, we maintain a per-listener backlog for every thread that listens on the socket.
The monitor distributes SYN commands into queues to listeners.
