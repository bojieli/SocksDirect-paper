...
In order to remove synchronization overhead for multi-thread applications, we regard all threads as a shared-nothing distributed system that communicates with message passing.
Although threads in a process share memory, we use thread-specific storage in \libipc{}.
...
All socket connections are shared among threads. Socket.
Therefore, a socket connection is a multiple producer and multiple consumer FIFO. With a traditional mutual exclusion solution, the multi-process scalability of socket is limited. Our aim is to maximize the common-case performance while keeping compatibility with Linux socket.
