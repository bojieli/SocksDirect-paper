...
To achieve high performance while preserving security, we design a \emph{monitor} daemon in each host to coordinate control plane operations, while establish queues between each pair of applications as data plane. The \emph{monitor} daemon is started at OS initialization time.
...
During initialization, \libipc{} connects to the local monitor via the shared memory queue.
...
For local peers, the monitor just construct a shared memory queue between the two applications (possibly in different containers), so that they can communicate directly.
