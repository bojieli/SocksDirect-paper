...
Before designing the connection management protocol, we keep the following requirements in mind: 1) The applications may be in an overlay network and needs address translation. 2) The applications and \libipc{} are not trusted, so we must enforce access control policies on the process boundary to prevent access to restricted resources. 3) Two processes may create multiple concurrent connections and should be accelerated. 4) Each address and port may be listened by multiple processes, which needs load balancing while avoiding starvation.
