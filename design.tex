...
It implements socket APIs in user space and forwards the other APIs to the kernel.
From a security point of view, because \libipc{} resides in the application address space, we cannot trust its behavior. %\RED{So what is your solution?}
Therefore we need a trusted component.
To achieve high performance while preserving security, we design a \emph{monitor} daemon in each host to coordinate control plane operations (e.g. connection creation), while establish queues between each pair of applications as data plane. The \emph{monitor} daemon is started at OS initialization time.
...
For local peers, the monitor just construct a shared memory queue between the two applications (possibly in different containers), so they can communicate directly.

When an application connects to a remote host, the monitor establishes a regular TCP connection and detects whether the remote host supports \sys{}.
