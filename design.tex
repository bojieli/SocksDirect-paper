...
It implements socket operations in user space and forwards other operations to the kernel.
...
If so, it establishes an RDMA queue between two monitors, so future connections between the two hosts can be established faster.
The monitor on the remote side dispatches the connection to the peer and helps the two applications establish an RDMA queue, as between host 1 and 2 in Figure~\ref{fig:architecture}.
