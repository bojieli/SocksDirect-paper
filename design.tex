...
Messages are stored back-to-back in the ring buffer. Each message in the ring buffer consists of an 8-byte header, a variably sized payload and an 8-byte trailer. The header consists of an \textit{isvalid} flag, an \textit{isdeleted} flag and the payload size. The payload is aligned to 8 bytes to accelerate memory copy. The trailer consists of a \textit{iscomplete} flag, an opcode and the receiver's FD number. Each byte in the ring buffer is initialized to zero. Sender writes the header, payload and trailer at the \textit{head} pointer sequentially, then advances \textit{head} to the end of message. Receiver polls \textit{isvalid} at \textit{tail}, checks \textit{iscomplete}, then copies the message and clears \textit{isvalid}, finally advances \textit{tail}.

\parab{Consistency between payload and metadata.}
For intra-server communication, one may think that out-of-order execution may mandate the use of memory fence instructions. Actually, memory fence is unnecessary because modern processors provide a strong inter-core memory consistency. X86 processors from Intel and AMD provides total store ordering~\cite{sewell2010x86,intel-manual}, which implies that two writes are observed in the same order as they were written, and that reads are never postponed in out-of-order execution. RDMA also ensures cache consistency on the receiver host~\cite{infiniband2000infiniband}. Because the sender writes \textit{isvalid}, payload, opcode, FD number and \textit{iscomplete} sequentially, the receiver would read a consistent message after polling \textit{isvalid} and \textit{iscomplete} flags.

\parab{Reduce polling overhead.}
Polling the inter-process queue wastes CPU cycles of the receiver when a pair of processes do not communicate frequently. For inter-server connections, a receiver uses the NIC to multiplex events in multiple inter-process queues. The sender uses RDMA \textit{write with immediate} and the receiver uses a \textit{shared RDMA completion queue} among all hosts. Because \libipc{} needs to issue RDMA receive work requests and the NIC needs to send completions, the throughput of the shared RDMA queue is 40\% lower than peer-to-peer RDMA queue. In light of this, the shared queue is only used for pairs of processes with low throughput.

For intra-server connections, we enable each inter-process queue to switch between \textit{polling} and \textit{interrupt} modes. The queue to the monitor process is always in polling mode. Receiver of each queue maintains a counter of consecutive empty polls. When it exceeds a threshold, the receiver sends a \textit{stop polling} request to sender and stops polling after receiving ACK. When sender writes to a sleeping queue, it sends an \textit{interrupt} message to the monitor and the monitor will signal the receiver to resume polling. 
...
\begin{figure}[t]
	...
	\caption{Zero copy via page remapping. Step 1: sender get physical address and send via shared memory. Step 2: receiver maps the page to buffer. Step 3: sender remaps buffer on write.}
	\vspace{-15pt}
	\label{fig:zerocopy}
\end{figure}
