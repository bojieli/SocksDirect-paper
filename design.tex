...
\RED{This section should be greatly simplified.}
...
When a non-designated receiver tries to \texttt{recv} from the socket, it broadcasts a \textit{takeover request} to all senders.
Upon receiving the request, each sender independently %processes takeover requests in FIFO order to avoid starvation and 
designates the requester as the new exclusive receiver and remove the status from the old designated receiver.
