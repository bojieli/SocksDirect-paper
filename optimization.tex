...
\parab{Batch page remapping.}
As Table~\ref{tab:operation-performance} shows, remapping a single page is more expensive than copying it because of kernel crossing and TLB flush costs. Hence, we batch page mapping operations to amortize the overhead.
We use zero copy for page-aligned \texttt{send} with at least 8~KiB data size.

\parab{Send pages securely in user space.}
...
\parab{Using RDMA NIC to send pages.}
On receiver, \libipc{} initializes a pinned page pool for the sender to use.
