...
%\libipc{} polls CQ (Completion Queue) to limit the number of in-flight (sent but not acknowledged) messages. This is not only required by RDMA NIC, but also enables \emph{adapative batching}~\cite{li2016clicknp,li2017kv}.
...
As shown in Figure~\ref{fig:zerocopy}, for \texttt{send} operation that is aligned to page boundary and at least 8~KiB, \libipc{} invokes the kernel to 
