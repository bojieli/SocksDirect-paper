...
This approach has been explored by zero-copy sockets~\cite{thadani1995efficient,chu1996zero,linux-zero-copy}, but still has several challenges.
...
As Table~\ref{tab:operation-performance} shows, remapping a single page is more expensive than copying it because of kernel crossing and TLB flush costs. Hence, we use zero copy only for \texttt{send} or \texttt{recv} with at least 8~KiB size.
We use a single syscall to remap multiple pages in one socket operation.
...
\parab{Using RDMA to send pages.}
