...
Another promising direction to solve the RDMA concurrency issue is to implement the reliable transport in CPU. However, RDMA UD does not support one-sided write, so the ring buffer in Sec.~\ref{subsec:lockless-queue} will not work. It would be interesting to improve software RDMA~\cite{soft-roce} or VMA on TCP/IP~\cite{libvma} for a better software-NIC co-design.
