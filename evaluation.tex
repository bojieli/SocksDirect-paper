...
For 8-byte messages, \sys achieves 0.3$\mu$s round-trip latency and up to 23~M messages per second throughput, close to raw SHM performance. For large messages, \sys uses zero copy to achieve 1/13 latency and 26x throughput than Linux.

\parab{Use RDMA efficiently for inter-host socket.}
\sys achieves 1.7$\mu$s round-trip latency, close to raw RDMA performance.
With zero copy, a single connection can saturate an 100~Gbps link.
