...
LibVMA and RSocket fall back to Linux for intra-host socket.
Using RDMA for inter-host socket, \sys achieves 62~M messages per second throughput (8x Linux and 24x LibVMA).
Although \sys{} creates $n^2$ queues for $n$ sender and receiver threads, only $n$ of them are active.
Because the NIC only caches active connections, the idle queues do not degrade performance.
Due to lock contention on shared NIC queues, compared to one thread, the throughput of LibVMA reduces to 1/4 with two threads, and 1/10 with three threads.
...
Second, although the latency increases almost linearly with number of active workers, but it is still 20x to 30x lower than Linux, because cooperative context switch is faster than kernel event signaling.
